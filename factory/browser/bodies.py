"""THE ONLY VENDOR-SEAM FILE IN THIS TREE.

Subclasses browser_use BaseWatchdog. Its BrowserStateSummary carries
`pending_network_requests`, which is what is in flight and never what came back;
Network.getResponseBody exists upstream only inside har_recording_watchdog.py.
...
#: Filters on HTTP metadata and never on content. A body is read because of what it is,
#: not because of what it says, so nothing here knows a site.
...
class Bodies:
    """What a page fetched for itself, on one CDP session."""
    ...
    def watch(self, cdp: Any) -> None:
        cdp.register.Network.responseReceived(self._noticed)

    def _noticed(self, message: dict[str, Any], session_id: str | None = None) -> None:
    ...
    async def drain(self, cdp: Any, session_id: str) -> list[Exchange]:
        ...
        different sizes, so the cap is applied to what actually came back rather than to a
        number CDP populates inconsistently.
        """
        ...
        for request_id, seen in taken.items():
            ...
            if worth_reading(seen["content_type"]):
                ...
                try:
                    got = await cdp.send.Network.getResponseBody(
